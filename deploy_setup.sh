...
# 1. Enable necessary APIs individually to avoid bash line-continuation errors
echo "Enabling APIs..."
...
for SECRET in "${SECRETS[@]}"; do
  if ! gcloud secrets describe $SECRET >/dev/null 2>&1; then
    ...
    echo "Created secret shell for $SECRET. Versioning will be added during key upload."
  else
...
echo "Granting Secret Accessor role to Compute Engine default service account: $COMPUTE_SERVICE_ACCOUNT"
for SECRET in "${SECRETS[@]}"; do
...
echo "Next Steps:"
echo "1. Upload actual API keys to Google Secret Manager (see docs/deployment_gcp.md)."
