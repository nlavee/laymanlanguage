...
export default function Home() {
  ...
  useEffect(() => {
    console.log("Home: Auth State", { isAuthLoading, hasToken: !!token });
    // Global redirect removed to allow guest exploration
  }, [token, isAuthLoading]);
  ...
  // Check if a valid profile with traits exists (if logged in)
  const hasProfile = !!token && profile && profile.metadata && profile.metadata.traits && Object.keys(profile.metadata.traits).length > 0;
        ...
        <div className="max-w-[1200px] mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            {token ? (
              <button 
                onClick={() => logout()}
                className="px-4 py-2 border border-neutral-800 hover:border-red-900/50 hover:bg-red-950/20 hover:text-red-400 text-sm font-medium rounded-lg transition-all flex items-center gap-2"
              >
                Sign Out
              </button>
            ) : (
